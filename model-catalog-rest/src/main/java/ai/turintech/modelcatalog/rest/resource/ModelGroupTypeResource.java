...
public class ModelGroupTypeResource {
   ...
   *     modelGroupTypeTO, or with status {@code 400 (Bad Request)} if the modelGroupType has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/model-group-types")
  public Mono<ResponseEntity<ModelGroupTypeTO>> createModelGroupType(
      @Valid @RequestBody ModelGroupTypeTO modelGroupTypeTO) throws URISyntaxException {
    log.debug("REST request to save ModelGroupType : {}", modelGroupTypeTO);
    if (modelGroupTypeTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new modelGroupType cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return modelGroupTypeFacade
        ...
        .map(
            result -> {
              try {
                return ResponseEntity.created(new URI("/api/model-group-types/" + result.getId()))
                    .headers(
   ...
   *     modelGroupTypeTO, or with status {@code 400 (Bad Request)} if the modelGroupTypeTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the modelGroupTypeTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  /**
   * {@code PATCH /model-group-types/:id} : Partial updates given fields of an existing
   * modelGroupType, field will ignore if it is null
   *
   * @param id the id of the modelGroupTypeTO to save.
   * @param modelGroupTypeTO the modelGroupTypeTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     modelGroupTypeTO, or with status {@code 400 (Bad Request)} if the modelGroupTypeTO is not
   *     valid, or with status {@code 404 (Not Found)} if the modelGroupTypeTO is not found, or with
   *     status {@code 500 (Internal Server Error)} if the modelGroupTypeTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/model-group-types/{id}",
  ...
  public Mono<ResponseEntity<ModelGroupTypeTO>> partialUpdateModelGroupType(
      ...
      throws URISyntaxException {
    log.debug(
        "REST request to partial update ModelGroupType partially : {}, {}", id, modelGroupTypeTO);
    if (modelGroupTypeTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, modelGroupTypeTO.getId())) {
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of modelGroupTypes
   *     in body.
   */
   ...
   *     modelGroupTypeDTO, or with status {@code 404 (Not Found)}.
   */
