    ...
    return (exchange, chain) -> {
      ...
      if (newLocale != null) {
        localeContextResolver.setLocaleContext(
            exchange, new SimpleLocaleContext(StringUtils.parseLocaleString(newLocale)));
      }
      // Proceed in any case.
  ...
  static class AngularCookieLocaleContextResolver implements LocaleContextResolver {
    ...
    private void addCookie(@Nonnull ServerHttpResponse response, String cookieValue) {
      ...
      ResponseCookie cookie =
          ResponseCookie.from(COOKIE_NAME, cookieValue).path(COOKIE_PATH).build();
    ...
    private void removeCookie(@Nonnull ServerHttpResponse response) {
      Assert.notNull(response, "ServerHttpResponse must not be null");
      ResponseCookie cookie =
          ResponseCookie.from(COOKIE_NAME, "").path(COOKIE_PATH).maxAge(Duration.ZERO).build();
      response.addCookie(cookie);
      if (logger.isDebugEnabled()) {
