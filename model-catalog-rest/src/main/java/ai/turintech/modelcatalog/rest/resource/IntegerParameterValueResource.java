...
public class IntegerParameterValueResource {
   ...
   *     integerParameterValueTO, or with status {@code 400 (Bad Request)} if the
   *     integerParameterValue has already an ID.
            ...
            result -> {
              try {
                return ResponseEntity.created(
                        new URI("/api/integer-parameter-values/" + result.getId()))
                    .headers(
   ...
   *     integerParameterValueDTO, or with status {@code 400 (Bad Request)} if the
   *     integerParameterValueTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the integerParameterValueTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  /**
   * {@code PATCH /integer-parameter-values/:id} : Partial updates given fields of an existing
   * integerParameterValue, field will ignore if it is null
   *
   * @param id the id of the integerParameterValueTO to save.
   * @param integerParameterValueTO the integerParameterValueDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     integerParameterValueTO, or with status {@code 400 (Bad Request)} if the
   *     integerParameterValueDTO is not valid, or with status {@code 404 (Not Found)} if the
   *     integerParameterValueDTO is not found, or with status {@code 500 (Internal Server Error)}
   *     if the integerParameterValueTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/integer-parameter-values/{id}",
  ...
  public Mono<ResponseEntity<IntegerParameterValueTO>> partialUpdateIntegerParameterValue(
      ...
      throws URISyntaxException {
    log.debug(
        "REST request to partial update IntegerParameterValue partially : {}, {}",
        id,
        integerParameterValueTO);
    if (integerParameterValueTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, integerParameterValueTO.getId())) {
   ...
   *     integerParameterValues in body.
   */
   ...
   *     integerParameterValueTO, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/integer-parameter-values/{id}")
  public Mono<ResponseEntity<IntegerParameterValueTO>> getIntegerParameterValue(
