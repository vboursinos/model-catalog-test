...
public class MetricResource {
   ...
   *     metricDTO, or with status {@code 400 (Bad Request)} if the metric has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/metrics")
  public Mono<ResponseEntity<MetricTO>> createMetric(@RequestBody MetricTO metricTO)
      throws URISyntaxException {
    log.debug("REST request to save Metric : {}", metricTO);
    if (metricTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new metric cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     metricDTO, or with status {@code 400 (Bad Request)} if the metricDTO is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the metricDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/metrics/{id}")
  public Mono<ResponseEntity<MetricTO>> updateMetric(
      @PathVariable(value = "id", required = false) final UUID id, @RequestBody MetricTO metricTO)
      throws URISyntaxException {
  ...
  /**
   * {@code PATCH /metrics/:id} : Partial updates given fields of an existing metric, field will
   * ignore if it is null
   *
   * @param id the id of the metricTO to save.
   * @param metricTO the metricTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     metricTO, or with status {@code 400 (Bad Request)} if the metricTO is not valid, or with
   *     status {@code 404 (Not Found)} if the metricTO is not found, or with status {@code 500
   *     (Internal Server Error)} if the metricTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/metrics/{id}",
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of metrics in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the metricTO, or
