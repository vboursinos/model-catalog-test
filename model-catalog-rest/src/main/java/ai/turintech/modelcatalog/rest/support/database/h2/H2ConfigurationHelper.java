 ...
 *
 * <p>We don't want to include H2 when we are packaging for the "prod" profile and won't actually
 * need it, so we have to load / invoke things at runtime through reflection.
 */
public class H2ConfigurationHelper {
  ...
  public static Object createServer(String port) throws SQLException {
    try {
      ...
      return createServer.invoke(
          null, new Object[] {new String[] {"-tcp", "-tcpAllowOthers", "-tcpPort", port}});
  ...
  /**
   * Init the H2 console via H2's webserver when no servletContext {@link
   * jakarta.servlet.ServletContext} is available.
   */
  ...
  static void initH2Console(String propertiesLocation) {
    try {
      ...
      Object server =
          createWebServer.invoke(
              null, new Object[] {new String[] {"-properties", propertiesLocation}});
      start.invoke(server);
    } catch (Exception e) {
  ...
  public static void initH2Console(ServletContext servletContext) {
    try {
      ...
      throw new RuntimeException(
          "Failed to load and initialize org.h2.server.web.JakartaWebServlet", e);
