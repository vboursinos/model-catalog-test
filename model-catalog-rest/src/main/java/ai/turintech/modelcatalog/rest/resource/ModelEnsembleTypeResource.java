...
public class ModelEnsembleTypeResource {
   ...
   *     modelEnsembleTypeTO, or with status {@code 400 (Bad Request)} if the modelEnsembleType has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/model-ensemble-types")
  public Mono<ResponseEntity<ModelEnsembleTypeTO>> createModelEnsembleType(
      @Valid @RequestBody ModelEnsembleTypeTO modelEnsembleTypeTO) throws URISyntaxException {
    log.debug("REST request to save ModelEnsembleType : {}", modelEnsembleTypeTO);
    if (modelEnsembleTypeTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new modelEnsembleType cannot already have an ID", ENTITY_NAME, "idexists");
    }
            ...
            result -> {
              try {
                return ResponseEntity.created(
                        new URI("/api/model-ensemble-types/" + result.getId()))
                    .headers(
   ...
   *     modelEnsembleTypeTO, or with status {@code 400 (Bad Request)} if the modelEnsembleTypeTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the modelEnsembleTypeTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  /**
   * {@code PATCH /model-ensemble-types/:id} : Partial updates given fields of an existing
   * modelEnsembleType, field will ignore if it is null
   *
   * @param id the id of the modelEnsembleTypeDTO to save.
   * @param modelEnsembleTypeTO the modelEnsembleTypeDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     modelEnsembleTypeDTO, or with status {@code 400 (Bad Request)} if the modelEnsembleTypeDTO
   *     is not valid, or with status {@code 404 (Not Found)} if the modelEnsembleTypeDTO is not
   *     found, or with status {@code 500 (Internal Server Error)} if the modelEnsembleTypeDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/model-ensemble-types/{id}",
  ...
  public Mono<ResponseEntity<ModelEnsembleTypeTO>> partialUpdateModelEnsembleType(
      ...
      throws URISyntaxException {
    log.debug(
        "REST request to partial update ModelEnsembleType partially : {}, {}",
        id,
        modelEnsembleTypeTO);
    if (modelEnsembleTypeTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
   ...
   *     modelEnsembleTypeDTO, or with status {@code 404 (Not Found)}.
   */
