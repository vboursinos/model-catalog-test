...
public class FloatParameterRangeResource {
   ...
   *     floatParameterRangeDTO, or with status {@code 400 (Bad Request)} if the floatParameterRange
   *     has already an ID.
  ...
  public Mono<ResponseEntity<FloatParameterRangeTO>> createFloatParameterRange(
      @Valid @RequestBody FloatParameterRangeTO floatParameterRangeTO) throws URISyntaxException {
            ...
            result -> {
              try {
                return ResponseEntity.created(
                        new URI("/api/float-parameter-ranges/" + result.getId()))
                    .headers(
   ...
   *     floatParameterRangeDTO, or with status {@code 400 (Bad Request)} if the
   *     floatParameterRangeDTO is not valid, or with status {@code 500 (Internal Server Error)} if
   *     the floatParameterRangeDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  /**
   * {@code PATCH /float-parameter-ranges/:id} : Partial updates given fields of an existing
   * floatParameterRange, field will ignore if it is null
   *
   * @param id the id of the floatParameterRangeDTO to save.
   * @param floatParameterRangeTO the floatParameterRangeDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     floatParameterRangeDTO, or with status {@code 400 (Bad Request)} if the
   *     floatParameterRangeDTO is not valid, or with status {@code 404 (Not Found)} if the
   *     floatParameterRangeDTO is not found, or with status {@code 500 (Internal Server Error)} if
   *     the floatParameterRangeDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/float-parameter-ranges/{id}",
  ...
  public Mono<ResponseEntity<FloatParameterRangeTO>> partialUpdateFloatParameterRange(
      ...
      throws URISyntaxException {
    log.debug(
        "REST request to partial update FloatParameterRange partially : {}, {}",
        id,
        floatParameterRangeTO);
    if (floatParameterRangeTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, floatParameterRangeTO.getId())) {
   ...
   *     floatParameterRangeDTO, or with status {@code 404 (Not Found)}.
   */
