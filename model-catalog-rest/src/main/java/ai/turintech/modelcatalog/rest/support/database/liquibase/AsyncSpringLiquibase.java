package ai.turintech.modelcatalog.rest.support.database.liquibase;
 ...
 *
 * <p>By default, this asynchronous version only works when using the "dev" profile.
 *
 * <p>The standard liquibase.integration.spring.SpringLiquibase starts Liquibase in the current
 * thread:
 *
 * <ul>
 *   <li>This is needed if you want to do some database requests at startup
 *   <li>This ensure that the database is ready when the application starts
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>On a recent MacBook Pro, start-up time is down from 14 seconds to 8 seconds
 *   <li>In production, this can help your application run on platforms like Heroku, where it must
 *       start/restart very quickly
 * </ul>
 */
public class AsyncSpringLiquibase extends DataSourceClosingSpringLiquibase {
  ...
  public void afterPropertiesSet() throws LiquibaseException {
    if (!env.acceptsProfiles(Profiles.of(ApplicationProfiles.SPRING_PROFILE_NO_LIQUIBASE))) {
      if (env.acceptsProfiles(
          Profiles.of(
              ApplicationProfiles.SPRING_PROFILE_DEVELOPMENT
                  + "|"
                  + ApplicationProfiles.SPRING_PROFILE_HEROKU))) {
