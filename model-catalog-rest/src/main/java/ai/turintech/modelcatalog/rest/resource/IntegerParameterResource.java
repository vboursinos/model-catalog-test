...
public class IntegerParameterResource {
  ...
  public Mono<ResponseEntity<IntegerParameterTO>> createIntegerParameter(
    ...
    return integerParameterFacade
        .save(integerParameterMapper.toDto(integerParameterTO))
        .map(integerParameterMapper::toTo)
        .map(
            result -> {
              try {
                return ResponseEntity.created(
                        new URI("/api/integer-parameters/" + result.getParameterTypeDefinitionId()))
                    .headers(
   ...
   *     integerParameterDTO, or with status {@code 400 (Bad Request)} if the integerParameterDTO is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the integerParameterDTO
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return integerParameterFacade
        ...
        .flatMap(
            exists -> {
              ...
              return integerParameterFacade
                  .update(integerParameterMapper.toDto(integerParameterTO))
                  .map(integerParameterMapper::toTo)
  ...
  /**
   * {@code PATCH /integer-parameters/:id} : Partial updates given fields of an existing
   * integerParameter, field will ignore if it is null
   *
   * @param id the id of the integerParameterDTO to save.
   * @param integerParameterTO the integerParameterDTO to update.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     integerParameterDTO, or with status {@code 400 (Bad Request)} if the integerParameterDTO is
   *     not valid, or with status {@code 404 (Not Found)} if the integerParameterDTO is not found,
   *     or with status {@code 500 (Internal Server Error)} if the integerParameterDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/integer-parameters/{id}",
        ...
        .flatMap(
            exists -> {
              ...
              Mono<IntegerParameterTO> result =
                  integerParameterFacade
                      .partialUpdate(integerParameterMapper.toDto(integerParameterTO))
                      .map(integerParameterMapper::toTo);
   ...
   *     integerParameterDTO, or with status {@code 404 (Not Found)}.
   */
  ...
  public Mono<ResponseEntity<IntegerParameterTO>> getIntegerParameter(@PathVariable UUID id) {
    ...
    Mono<IntegerParameterTO> integerParameterTO =
        integerParameterFacade.findOne(id).map(integerParameterMapper::toTo);
