...
public class ModelTypeResource {
   ...
   *     ModelTypeTO, or with status {@code 400 (Bad Request)} if the modelType has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/model-types")
  public Mono<ResponseEntity<ModelTypeTO>> createModelType(
      @Valid @RequestBody ModelTypeTO modelTypeTO) throws URISyntaxException {
    log.debug("REST request to save ModelType : {}", modelTypeTO);
    if (modelTypeTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new modelType cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return modelTypeFacade
        ...
        .map(
            result -> {
              try {
                return ResponseEntity.created(new URI("/api/model-types/" + result.getId()))
                    .headers(
   ...
   *     ModelTypeTO, or with status {@code 400 (Bad Request)} if the ModelTypeTO is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the ModelTypeTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  ...
  /**
   * {@code PATCH /model-types/:id} : Partial updates given fields of an existing modelType, field
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     ModelTypeTO, or with status {@code 400 (Bad Request)} if the ModelTypeTO is not valid, or
   *     with status {@code 404 (Not Found)} if the ModelTypeTO is not found, or with status {@code
   *     500 (Internal Server Error)} if the ModelTypeTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/model-types/{id}",
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of modelTypes in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the ModelTypeTO,
