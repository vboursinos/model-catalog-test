...
/**
 * ResponseStatusException handler for springwebflux(reactive) application that will facilitate the
 * translation to ProblemDetail
 */
public class ReactiveWebExceptionHandler
  ...
  public Mono<Void> handle(final ServerWebExchange exchange, final Throwable throwable) {
    if (throwable instanceof ResponseStatusException) {
      final Mono<ResponseEntity<Object>> entityMono =
          translator.handleAnyException(throwable, exchange);
      return entityMono.flatMap(entity -> this.setHttpResponse(entity, exchange, mapper));
    }
    return Mono.error(throwable);
  }

  private Mono<Void> setHttpResponse(
